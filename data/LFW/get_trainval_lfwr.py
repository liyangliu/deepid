...
files_ali = pd.read_csv('aux/SeetaAli.txt', sep = ' ')
...
if not cropped:
    files_det = pd.read_csv('aux/SeetaDet.txt', sep = ' ')
    sorted_det = files_det.sort(columns = 'img_id')

crop_root = 'LFW_crop/'
ftrain = open(r'train_rep.txt', 'w')
    ...
    if person != person_before:
        ...
        for j in range(num_images_person):
            ...
            if r <= per_th:
                ...
                for k in range(rep//num_images_person+1):
